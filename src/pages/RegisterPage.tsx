...
export default function RegisterPage() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    e.preventDefault();
    setError(null);
    if (!username.trim() || !password) { setError("Usuario y contraseña son obligatorios."); return; }
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new ApiError(data?.title ?? `Error ${response.status}`, response.status);
      }
  ...
  return (
    <IonPage>
      <IonHeader><IonToolbar><IonTitle>Registro — Fya Créditos</IonTitle></IonToolbar></IonHeader>
      <IonContent className="ion-padding">
        <form onSubmit={handleSubmit}>
          <IonItem>
            <IonLabel position="stacked">Usuario</IonLabel>
            <IonInput value={username} onIonInput={(e) => setUsername(e.detail.value ?? "")} autocapitalize="off" />
          </IonItem>
          <IonItem>
          ...
          {error && <IonText color="danger"><p className="ion-padding-top">{error}</p></IonText>}
